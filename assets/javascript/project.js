$(document).ready(function () {
    ...
    rankRef.orderByChild('popularity').limitToLast(3).on('value', function (snapshot) {
        ...
        for (var j = 0; j < topThree.length; j++) {
            ...
            printButtons.attr("artist", topThree[j].artist);
            printButtons.addClass("firebaseArtists");
    ...
    $(document.body).on("click", ".firebaseArtists", function (artist) {
        event.preventDefault();

        //console.log("hello");

        let artistNameFire = $(this).attr("artist");
        let firebaseArtist = artistNameFire.replace(" ", "+");
        console.log(firebaseArtist);

        getArtistInfo(firebaseArtist);
        
        //console.log(firebaseArtist);
        getArtistBio(firebaseArtist);
        ...
        artistInformation(firebaseArtist);
        events(firebaseArtist);
    ...
    function artistInformation(artist) {
        ...
        var queryURL = "https://cors-anywhere.herokuapp.com/https://rest.bandsintown.com/artists/" + artist + "?app_id=codingbootcamp";
        $.ajax({
            url: queryURL,
            method: "GET"
    ...
    function events(artist) {
        var queryURL = "https://cors-anywhere.herokuapp.com/https://rest.bandsintown.com/artists/" + artist + "/events?app_id=codingbootcamp";
        $.ajax({
            url: queryURL,
            method: "GET"
        }).then(function (response) {
